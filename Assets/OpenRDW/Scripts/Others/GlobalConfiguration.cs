...
public class GlobalConfiguration : MonoBehaviour
{
    ...
    {        
        ...
        // 安全地查找相机，如果不存在则不设置
        Transform virtualTopCamTransform = transform.Find("Virtual Top View Cam For All Avatars");
        if (virtualTopCamTransform != null)
        {
            cameraVirtualTopForAllAvatars = virtualTopCamTransform.GetComponent<Camera>();
        }
        else
        {
            cameraVirtualTopForAllAvatars = null;
            Debug.LogWarning("GlobalConfiguration: 'Virtual Top View Cam For All Avatars' not found. This is normal for threshold measurement experiments.");
        }
        
        // 安全地查找虚拟世界，如果不存在则不设置
        Transform virtualWorldTransform = transform.Find("Virtual World");
        if (virtualWorldTransform != null)
        {
            virtualWorld = virtualWorldTransform.gameObject;
        }
        else
        {
            virtualWorld = null;
            Debug.LogWarning("GlobalConfiguration: 'Virtual World' not found. This is normal for threshold measurement experiments.");
        }
